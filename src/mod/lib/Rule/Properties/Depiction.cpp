...
template<Membership membership>
std::string DepictionDataCore::DepictionData<membership>::getVertexLabelNoCharge(Vertex v) const {
...
DepictionDataCore::DepictionDataCore(const GraphType &g, const PropStringCore &labelState, const PropMoleculeCore &moleculeState)
: g(g), moleculeState(moleculeState), hasMoleculeEncoding(true), hasCoordinates(false) {
	{ // vertexData
		...
		for(auto p : verticesToProcess) {
			...
			auto label = Chem::extractCharge(m == Membership::Left ? labelState.getLeft()[v] : labelState.getRight()[v]).first;
			...
			if(m == Membership::Left) {
				auto charge = moleculeState.getLeft()[v].getCharge();
				nonAtomToPhonyAtomLeft[v] = AtomData(atomId, charge);
			} else {
				auto charge = moleculeState.getRight()[v].getCharge();
				nonAtomToPhonyAtomRight[v] = AtomData(atomId, charge);
			}
		}
	}
	{ // edgeData
