...
struct PredSpec {
	...
	bool operator()(const VEDom &veDom, const VECodom &veCodom, const LabGraphDom &gDom, const LabGraphCodom &gCodom) const {
		...
		const auto &pDom = get_stereo(gDom);
		const auto &pCodom = get_stereo(gCodom);
		const bool res = Handler::reduce(std::logical_and<>(), Handler::fmap2(get(pDom, veDom), get(pCodom, veCodom), gDom, gCodom, LocalSpec()));
		return res && next(veDom, veCodom, gDom, gCodom);
	}
private:
	Next next;
...
struct StereoDataSpec {
	...
	get_geometry(const StereoDataSpec&, const LabGraphLeft &lgLeft, const LabGraphRight &lgRight, const VertexRight &vRight) {
	...
	get_edge_category(const StereoDataSpec&, const LabGraphLeft &lgLeft, const LabGraphRight &lgRight, const EdgeRight &eRight) {
...
template<typename VertexMap, typename GraphMorDom, typename GraphMorCodom, typename LabGraphDom, typename LabGraphCodom>
std::vector<std::size_t> makePermutation(const auto &vDom, const auto &vCodom, const VertexMap &m, const GraphMorDom &gMorDom, const GraphMorCodom &gMorCodom,
		const ConfPtr &cDom, const ConfPtr &cCodom, const LabGraphDom &gLabDom, const LabGraphCodom &gLabCodom) {
	...
	for(std::size_t i = 0; i < lDeg; ++i) {
		...
		case EmbeddingEdge::Type::Edge:
		{
			...
			if(vOutRight == boost::graph_traits<GraphMorCodom>::null_vertex()) {
				...
				const auto oeRightIter = std::find_if(oesRight.first, oesRight.second, [vOutRight, &gCodom](const auto &eOutRight) {
			...
			break;
		case Stereo::EmbeddingEdge::Type::LonePair:
			perm[i] = -1; // to be mapped later
			break;
		case Stereo::EmbeddingEdge::Type::Radical:
			MOD_ABORT;
...
struct ToVertexMapIso : ToVertexMapBase<ToVertexMapIso<LabGraphDom, LabGraphCodom, Next>, LabGraphDom, LabGraphCodom, Next> {
...
private:
	...
	bool unifyEdges(const EdgeDom &eDom, const EdgeCodom &eCodom, const PropDom &pDom, const PropCodom &pCodom) const {
		return Handler::reduce(std::logical_and<>(), Handler::fmap2(get(pDom, eDom), get(pCodom, eCodom), this->lgDom, this->lgCodom, LocalIso()));
	}

	template<typename VertexDom, typename VertexCodom, typename PropDom, typename PropCodom>
	bool unifyGeometries(const VertexDom &vDom, const VertexCodom &vCodom, const PropDom &pDom, const PropCodom &pCodom) const {
		return Handler::reduce(std::logical_and<>(), Handler::fmap2(get(pDom, vDom), get(pCodom, vCodom), this->lgDom, this->lgCodom, LocalIso()));
	}
	...
	bool mapNeighbourhoods(const VertexDom &vDom, const VertexCodom &vCodom, const PropDom &pDom, const PropCodom &pCodom,
			const VertexMap &m, const GraphMorDom &gMorDom, const GraphMorCodom &gMorCodom) const {
		const auto mapper = [&vDom, &gMorDom, &vCodom, &gMorCodom, &m](const ConfPtr &cDom, const ConfPtr &cCodom, const auto &gLabDom, const auto &gLabCodom) {
		...
		return Handler::reduce(std::logical_and<>(), Handler::fmap2(get(pDom, vDom), get(pCodom, vCodom), this->lgDom, this->lgCodom, vMapper));
	}

	template<typename VertexMap, typename PropDom, typename PropCodom, typename GraphMorDom, typename GraphMorCodom>
	auto wrapVertexMap(VertexMap &&m, const PropDom &pDom, const PropCodom &pCodom, const GraphMorDom &gMorDom, const GraphMorCodom &gMorCodom) const {
...
template<typename LabGraphDom, typename LabGraphCodom, typename Next>
struct ToVertexMapSpec : ToVertexMapBase<ToVertexMapSpec<LabGraphDom, LabGraphCodom, Next>, LabGraphDom, LabGraphCodom, Next> {
...
private:
	...
	bool unifyEdges(const EdgeDom &eDom, const EdgeCodom &eCodom, const PropDom &pDom, const PropCodom &pCodom) const {
		...
		return Handler::reduce(std::logical_and<>(), Handler::fmap2(propDom, propCodom, this->lgDom, this->lgCodom, LocalSpec()));
	}

	template<typename VertexDom, typename VertexCodom, typename PropDom, typename PropCodom>
	bool unifyGeometries(const VertexDom &vDom, const VertexCodom &vCodom, const PropDom &pDom, const PropCodom &pCodom) const {
		return Handler::reduce(std::logical_and<>(), Handler::fmap2(get(pDom, vDom), get(pCodom, vCodom), this->lgDom, this->lgCodom, LocalSpec()));
	}
