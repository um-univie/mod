#ifndef JLA_BOOST_GRAPH_MORPHOSM_COMMON_SUBGRAPH_HPP
#define JLA_BOOST_GRAPH_MORPHOSM_COMMON_SUBGRAPH_HPP

// Adapted from Boost.Graph mcgregor_common_subgraph

#include <jla_boost/graph/morphism/InjectiveEnumeration.hpp>

#include <jla_boost/Functional.hpp>
#include <jla_boost/graph/PairToRangeAdaptor.hpp>
#include <jla_boost/graph/morphism/InvertibleVertexMapAdaptor.hpp>
#include <jla_boost/graph/morphism/PreimageSizedVertexMap.hpp>
	...
	bool operator()(Callback &&callback) {
		...
		for(auto vLeft : vsLeft) {
			...
			for(auto vRight : vsRight) {
				...
				auto mSized = makePreimageSizedVertexMap(this->getVertexMap(), this->getStackSize());
				if(!callback(mSized, this->gLeft, this->gRight)) {
					return false;
				}
...
public:
	...
	bool operator()(VertexMap &&m, const GraphLeft &gLeft, const GraphRight &gRight) {
		auto subgraphSize = get_preimage_size(m);
		if(Maximum) {
			if(!cache.empty()) {
	...
	void outputSubgraphs() {
		...
		for(auto &subgraph : cache)
			next(makePreimageSizedVertexMap(std::move(subgraph.m), subgraph.size), gLeft, gRight);
	}
private:
...
#endif /* JLA_BOOST_GRAPH_MORPHOSM_COMMON_SUBGRAPH_HPP */