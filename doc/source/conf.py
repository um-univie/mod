...
""")
	for name, link in sorted(refLinks.items(), key=lambda x: x[0].lower()):
		f.write("* " + name + " " + link + "\n")
...
def generateExamples():
	import json
	with open("_static/examples/py/sections.json") as j:
		sections = json.load(j)
	with open("examples/inc_content.rst", "w") as f:
		f.truncate()
		f.write(".. Autogenerated in conf.py\n\n")
		f.write(".. toctree::\n\n")
		for s in sections:
			f.write("\tinc_{}\n".format(s["id"]))
			s["f"] = open("examples/inc_{}.rst".format(s["id"]), "w")
			s["f"].truncate()
			s["f"].write(".. Autogenerated in conf.py\n\n")
			if "future" in s and s["future"]:
				s["f"].write("Upcomming Feature: {}\n".format(s["title"]))
			else:
				s["f"].write("{}\n".format(s["title"]))
			s["f"].write(80*"#" + "\n\n")

	for fName in sorted(os.listdir("_static/examples/py/")):
		if not fName.endswith(".py"):
			continue
		assert fName[3] == '_'
		id_ = int(fName[:3])		
		s = next(s for s in sections
				if id_ >= int(s["first"]) and id_ < int(s["last"]))

		with open("_static/examples/py/{}".format(fName)) as f:
			endLine = 1
...
<https://cheminf-live.imada.sdu.dk/mod?example={}>`__.

.. literalinclude:: /_static/examples/py/{}
	:language: python
	:linenos:
	:lines: 1-{}
	:tab-width: 3
""".format(fName, fName, endLine - 1))
...
project = u'MØD'
copyright = u'2013-2020, Jakob Lykke Andersen'
