#ifndef MOD_LIB_IO_PARSING_HPP
#define MOD_LIB_IO_PARSING_HPP
...
std::string
makeParserError(const TextIter &textIter, const PosIter &iter, const PosIter &iterEnd, const bool withPosition) {
	...
	if(withPosition)
		msg += " at " + std::to_string(lineNumber) + ":" + std::to_string(column);
...
std::string makeParserExpectationError(const x3::expectation_failure<PosIter> &e, const TextIter &textIter,
                                       const PosIter &iterEnd, const bool withPosition) {
...
void parse(const TextIter &textFirst, PositionIter<TextIter> &first, const PositionIter<TextIter> &last,
           const Parser &p, Attr &attr, const bool withPosition, const Skipper &...skipper) {
	bool res;
	try {
		...
		throw ParsingError{detail::makeParserExpectationError(e, textFirst, last, withPosition)};
	}
	if(!res || first != last)
		throw ParsingError{detail::makeParserError(textFirst, first, last, withPosition)};
}
...
void parse(const TextIter &textFirst, const TextIter &textLast,
	...
	return parse(textFirst, first, PositionIter<TextIter>(textLast), p, attr, withPosition, skipper...);
}

} // namespace mod::lib::IO

#endif // MOD_LIB_IO_PARSING_HPP