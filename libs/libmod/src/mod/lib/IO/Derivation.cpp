...
void forEachMatch(const lib::DG::NonHyper &dg, lib::DG::HyperVertex v, const lib::Rules::Real &rReal, F f) {
	...
	if(!derivationFound)
		IO::log() << "WARNING: derivation " << get(boost::vertex_index_t(), dgGraph, v) << " does not exist."
					 << std::endl;
}
...
summary(const lib::DG::NonHyper &dg,
	...
	for(const lib::Rules::Real *rPtr : rules) {
		...
		const auto f = [&res, &options, &nomatchColour, &matchColour, &rUpperCopy](
			...
			if(!getConfig().dg.dryDerivationPrinting.get()) {
				...
				std::string fileNoExt1 = IO::Rules::Write::pdf(rUpperCopy, options, strMatch + "_derL", strMatch + "_derK",
																			  strMatch + "_derR",
																			  IO::Rules::Write::BaseArgs{visibleRule, vColourRule,
																												  eColourRule});
				std::string fileNoExt2 = IO::Rules::Write::pdf(rLower, options, strMatch + "_derG", strMatch + "_derD",
																			  strMatch + "_derH",
																			  IO::Rules::Write::BaseArgs{visibleInstantiation,
																												  vColourInstantiation,
																												  eColourInstantiation});
...
void summaryTransitionState(const lib::DG::NonHyper &dg,
									 lib::DG::HyperVertex v,
									 const IO::Graph::Write::Options &options) {}
