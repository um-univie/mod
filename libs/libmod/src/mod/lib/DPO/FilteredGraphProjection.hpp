#ifndef MOD_LIB_DPO_FILTERED_GRAPH_PROJECTION_HPP
#define MOD_LIB_DPO_FILTERED_GRAPH_PROJECTION_HPP
...
struct FilteredGraphProjection {
	...
	struct Filter {
		...
		bool operator()(typename boost::graph_traits<GraphType>::vertex_descriptor v) const {
			...
			return m == membership || m == Membership::K;
		}
...
public:
	explicit FilteredGraphProjection(const GraphType &gInner, Membership membership)
...
template<typename GraphType>
struct GraphAdaptorTraits<mod::lib::DPO::FilteredGraphProjection<GraphType> > {
	using type = GraphType;

	static const type &unwrap(const mod::lib::DPO::FilteredGraphProjection<GraphType> &g) {
		return g.gInner;
	}
};
...
struct property_map<mod::lib::DPO::FilteredGraphProjection<GraphType>, Property>
		: property_map<GraphType, Property> {
};

template<typename GraphType, typename Property>
struct property_map<const mod::lib::DPO::FilteredGraphProjection<GraphType>, Property>
		: property_map<GraphType, Property> {
};

} // namespace boost

#endif // MOD_LIB_DPO_FILTERED_GRAPH_PROJECTION_HPP