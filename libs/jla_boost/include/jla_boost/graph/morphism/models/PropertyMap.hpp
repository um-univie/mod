...
struct MatchAsVertexMap {
	...
	reinterpret(MatchAsVertexMap<GraphDom, GraphCodom, Map, MapInv> &&m, const GraphDom &, const GraphCodom &,
	            const GraphDomU &, const GraphCodomU &) {
...
public:
	...
	get_inverse(const MatchAsVertexMap<GraphDom, GraphCodom, Map, MapInv> &m, const GraphDom &gDom,
...
struct MatchAsVertexMapWrapper {
	MatchAsVertexMapWrapper(const GraphDom &gDom, const GraphCodom &gCodom, Next next)
			: gDom(gDom), gCodom(gCodom), next(next) {}
...
template<typename GraphDom, typename GraphCodom, typename Next>
MatchAsVertexMapWrapper<GraphDom, GraphCodom, Next>
makeMatchAsVertexMapWrapper(const GraphDom &gDom, const GraphCodom &gCodom, Next next) {
